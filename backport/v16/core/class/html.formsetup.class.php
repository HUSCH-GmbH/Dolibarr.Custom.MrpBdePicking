...
class FormSetup
{
	...
	public function addItemFromParams($confKey, $params)
	{
		 ...
		 * 	// 'MRPBDEPICKING_MYPARAM1'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
			// 'MRPBDEPICKING_MYPARAM2'=>array('type'=>'textarea','enabled'=>1),
			//'MRPBDEPICKING_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
			//'MRPBDEPICKING_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
			//'MRPBDEPICKING_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
			//'MRPBDEPICKING_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
			//'MRPBDEPICKING_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
			//'MRPBDEPICKING_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
		 */
		...
		if (!empty($TSelected)) {
			foreach ($TSelected as $selected) {
				if (!empty($this->fieldOptions[$selected])) {
					$outPut.= dolGetBadge('', $this->fieldOptions[$selected], 'info') . ' html.formsetup.class.php';
				}
			}
		}
		return $outPut;
